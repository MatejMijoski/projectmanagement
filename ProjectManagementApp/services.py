    ...
    for email in emails:
        ...
        try:
            invite = ProjectInvite.objects.get(
                project=project, email=email, sender=sender
            )
            ...
            if invite.expire_at < datetime.today():
                try:
                    ...
                    response.append(
                        {
                            "message": "An invite has been sent to the user with an email {}.".format(
                                email
                            )
                        }
                    )
                    ...
                    send_email(
                        "You have been added to a project.",
                        "Please create an account to be able to access the project.",
                        sender.email,
                        [
                            email,
                        ],
                ...
                response.append(
                    {
                        "message": "A new invite has been sent to the user with an email {}.".format(
                            email
                        )
                    }
                )
                ...
                response.append(
                    {
                        "message": "The user with an email {} has already been added.".format(
                            email
                        )
                    }
                )
        except ProjectInvite.DoesNotExist:
            project_invite = ProjectInvite.objects.create(
                project=project, email=email, sender=sender
            )
            try:
                Account.objects.get(email=email)
                send_email(
                    "You have been added to a project",
                    "Click on the link to accept the invite - {}.".format(
                        settings.SITE_URL + "/invite/" + str(project_invite.id)
                    ),
                    sender.email,
                    [
                        email,
                ...
                response.append(
                    {
                        "message": "The user with an email {} has been succesfully invited.".format(
                            email
                        )
                    }
                )
...
def accept_project_invite(user, invite_id=None):
    ...
    if invite_id:
        try:
            ...
            return {
                "message": "You have succesfully been added to the project {}.".format(
                    invite.project.name
                )
            }
        except ProjectInvite.DoesNotExist:
    ...
    # If the invite_id is None, this function has been called for Sign Up
        ...
        for invite in invites:
            if invite.add_user(user):
                response.append(
                    {
                        "message": "You have been added to project {}".format(
                            invite.project.name
                        )
                    }
                )
                ...
                response.append(
                    {
                        "message": "There was a problem with adding you to the project {}. "
