...
from ProjectManagementApp.models import ProjectInvites
...
def send_invite_project(emails, project, sender):
    ...
    for email in emails:
        ...
        try:
            invite = ProjectInvites.objects.get(project=project, email=email, sender=sender)
            ...
            try:
                ...
                send_email("You have been added to a project.",
                           "Please create an account to be able to access the project.", sender.email, [email, ])
                response.append({"message": "An invite has been sent to the user with an email {}.".format(email)})
                ProjectInvites.objects.create(project=project, email=email, sender=sender)
...
def accept_project_invite(user, invite_id=None):
    ...
    if invite_id:
        try:
            invite = ProjectInvites.objects.get(id=invite_id, email=user.email)
    ...
    # If the invite_id is None, this function has been called for Sign Up
        ...
        invites = ProjectInvites.objects.filter(email=user.email)
        response = []
