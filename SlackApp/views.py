...
def slack_signup(request):
    response = requests.get('https://slack.com/api/oauth.v2.access?client_id=' + settings.client_id +
                            '&client_secret=' + settings.client_secret + '&code=' + request.GET.get('code', '')
                            + '&redirect_uri=' + urllib.parse.quote('http://localhost:8000/api/auth/slack/callback'))
    json_response = json.loads(response.content)

    # Continue only if this is true
    if json_response["ok"]:
        responseEmail = requests.get('https://slack.com/api/users.identity',
                                     headers={
                                         'Authorization': "Bearer " + json_response['authed_user']['access_token']},
                                     )
        ...
        try:
            ...
            account = Account.objects.create(email=emailJson["user"]["email"], password=hashers.make_password(None),
                                             email_verified=True)
            slack_account = Slack_Auth.objects.create(user_id=json_response['authed_user']['id'],
            ...
            return JsonResponse(
                data={"slack_user_id": slack_account.slack_id, "user_id": slack_account.user_id,
...
def slack_signin(request):
    ...
    try:
        slack_account = Slack_Auth.objects.get(slack_id=data["slack_user_id"], user_id=data["user_id"])
...
def slack_authorization(request):
    response = requests.get('https://slack.com/api/oauth.v2.access?client_id=' + settings.client_id +
        ...
        'http://localhost:8000/api/slack/permissions/callback'))
    json_response = json.loads(response.content)
    # Continue only if this is true
    if json_response['ok']:
...
def get_slack_channels(request):
    data = request.data
    try:
        user = Slack_Auth.objects.get(slack_account=request.user, slack_id=data["slack_user_id"],
...
def slack_pagination(request):
    if request.GET.get("cursor", "") != "" and request.GET.get("channel-id", "") != "" and request.GET.get("user-id",
                                                                                                           "") != "":
        try:
            user = Slack_Auth.objects.get(slack_account=request.user, user_id=request.GET.get("user-id", ""))
            response = requests.get(
                'https://slack.com/api/conversations.history?channel=' + request.GET.get("channel-id", "")
    ...
    if data["ts"] != "" and data["updated_message"] != "" and data["channel-id"] != "":
        try:
            ...
            response = requests.post('https://slack.com/api/chat.update',
                                     data={"channel": data["channel-id"], "ts": data["ts"],
            ...
            if responseJson["ok"]:
                ...
                return JsonResponse(data={"error": "The message doesn't exist."}, status=400)
            elif responseJson["error"] == "cant_update_message":
                return JsonResponse(data={"error": "You don't have permissions to edit this message."}, status=400)
            elif responseJson["error"] == "update_failed":
                return JsonResponse(data={"error": "The Slack server is not available. Please try again later"},
                                    status=500)
            elif responseJson["error"] == "msg_too_long":
                return JsonResponse(data={"error": "The message is too long"},
                                    status=400)
            else:
                return JsonResponse(data={"error": "The message can not be updated. Please try again later or"
                                                   "try the Slack App."}, status=500)
        except Slack_Auth.DoesNotExist:
            return Response(status=404)
    return Response(status=400)
    ...
    if request.GET.get("ts", "") != "":
        try:
            ...
            if data["ok"]:
                ...
                return Response(data=json.dumps({"data": "The message was not found"}), status=404)
        except Slack_Auth.DoesNotExist:
            return Response(status=404)
    return Response(status=400)
            ...
            for file in files:
                ...
                if not responseJson["ok"]:
                    if responseJson["error"] == "posting_to_general_channel_denied":
                        responseJson.update({"error": "The admin has restricted posting."})
                    elif responseJson["error"] == "invalid_channel":
                        responseJson.update({"error": "The channel is not valid."})
                    elif responseJson["error"] == "slack_connect_file_upload_sharing_blocked":
                        responseJson.update({"error": "The admin has restricted file uploads."})
                    else:
                        responseJson.update(
...
def slack_file_delete(request):
    ...
    try:
        ...
        for file in data["file"]:
            response = requests.post('https://slack.com/api/files.delete',
                                     data={"file": file},
                                     headers={"Authorization": "Bearer " + user.access_token})
            responseJson = json.loads(response.content.decode("UTF-8"))
            try:
