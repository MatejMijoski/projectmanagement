...
class TaskListCreateView(ListCreateAPIView):
    ...
    def get_queryset(self):
        return ProjectTasks.objects.filter(Q(project__owner=self.request.user) | Q(project__owner=self.request.user),
...
class TaskRetrieveView(RetrieveUpdateDestroyAPIView):
    ...
    def get_object(self):
        return ProjectTasks.objects.get(id=self.kwargs.get('id'), owner=self.request.user)
