...
def check_file_if_exists(file_name):
    ...
    content = s3.list_objects_v2(Bucket=settings.AWS_STORAGE_BUCKET_NAME, Prefix=file_name)
...
def upload_file(data, file_path):
    ...
    if file.size > 2000000:
        return 400, {"error": "The maximum file size is 2 MB. The current file is " + "{:.0f}".format(
    ...
    if check_file_if_exists(file_path):  # avoid overwriting existing file
        ...
        try:
            ...
            raise CustomException(400, "There was an error with the request. Please try again later.", e)
        buffer.detach()
